...
[![GitHub tag (latest SemVer)](https://img.shields.io/github/tag/leroy20317/subconverter.svg)](https://github.com/leroy20317/subconverter/tags)
...
| ------------- | :----: | :--------------------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| target        |  必要  | surge&ver=4                  | 指想要生成的配置类型，详见上方 [支持类型](#支持类型) 中的参数                                                                                                                                                                                                                            |
| url           |  可选  | https%3A%2F%2Fwww.xxx.com    | 指机场所提供的订阅链接或代理节点的分享链接，需要经过 [URLEncode](https://www.urlencoder.org/) 处理，**可选的前提是在 `default_url` 中进行指定**。也可以使用 data URI。可使用 `tag:xxx,https%3A%2F%2Fwww.xxx.com` 指定该订阅的所有节点归属于`xxx`分组，用于配置文件中的`!!GROUP=XXX`、`!!TAG`、`%TAG%` 等匹配；未设置 tag 时不会再导出空的 group 字段 |
...
在程序目录内的任意位置创建一个新的文档文件（推荐保存至 `profiles` 文件夹内，以使整洁目录及便于后续维护），如 `formyairport.ini`，并仿照仓库中的 `base/profiles/example_profile.ini` 根据配置好的参数填写进去即可。
...
注：本部分内容以本程序中的 `base/pref.example.ini`、`base/pref.example.yml` 或 `base/pref.example.toml` 为准，本文档可能由于更新不及时，内容不适用于新版本。
...
10. **clash_proxies_style**
    ...
    - 风格示例：
      ...
      Compact: [{ name: name1, key: value }, { name: name2, key: value }]
      ```
...
- 还可使用一些特殊筛选条件：
  ...
  `` `!!UNTAGGED`` 仅匹配未使用 `tag:xxx,link` 标记的节点

  `` `!!TAG`` 在 `%TAG%` 自动分组模板中匹配当前 tag 对应的所有节点

  `` `!!IN=Group_A,Group_B`` 在 `%TAG%` 自动分组模板中，将生成出来的 tag 分组自动挂载到指定父组中
  ...
  # 表示创建一个叫 default 的 select 策略组,并向其中依次添加所有未打 tag 的节点
...
注：本部分内容以本程序中的 `base/config/example_external_config.ini`、`base/config/example_external_config.yml` 或 `base/config/example_external_config.toml` 为准，本文档可能由于更新不及时，内容不适用于新版本。
