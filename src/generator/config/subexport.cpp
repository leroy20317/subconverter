...
std::string buildExactGroupMatcher(const std::string &group) {
    return "!!TAG=^" + escapeRegexLiteral(group) + "$";
}

string_array collectNodeTags(const std::vector<Proxy> &nodes) {
    string_array tags;
    for (const auto &node: nodes) {
        if (!node.Tag.empty() && std::find(tags.begin(), tags.end(), node.Tag) == tags.end())
            tags.emplace_back(node.Tag);
    }
    return tags;
}

bool isAutoTagTemplateGroup(const ProxyGroupConfig &group) {
    if (strFind(group.Name, "%TAG%"))
        return true;

    return std::any_of(group.Proxies.begin(), group.Proxies.end(), [](const std::string &rule) {
        return strFind(rule, "%TAG%") || startsWith(rule, "!!TAG");
    });
}

ProxyGroupConfigs buildProxyGroups(const ProxyGroupConfigs &source_groups, const std::vector<Proxy> &nodes) {
    ProxyGroupConfigs expanded_groups;
    std::vector<std::pair<std::string, std::string>> parent_links;
    string_array tags = collectNodeTags(nodes);

    for (const auto &group: source_groups) {
        if (!isAutoTagTemplateGroup(group)) {
            expanded_groups.push_back(group);
            continue;
        }

        for (const auto &tag: tags) {
            ProxyGroupConfig generated_group = group;
            generated_group.Name = replaceAllDistinct(group.Name, "%TAG%", tag);
            generated_group.Proxies.clear();

            for (const auto &raw_rule: group.Proxies) {
                std::string rule = replaceAllDistinct(raw_rule, "%TAG%", tag);

                if (startsWith(rule, "!!IN=")) {
                    for (const auto &parent_name: split(rule.substr(5), ",")) {
                        std::string trimmed_parent = trim(parent_name);
                        if (!trimmed_parent.empty())
                            parent_links.emplace_back(trimmed_parent, generated_group.Name);
                    }
                    continue;
                }

                if (rule == "!!TAG")
                    rule = buildExactGroupMatcher(tag);
                else if (startsWith(rule, "!!TAG!!"))
                    rule = buildExactGroupMatcher(tag) + "!!" + rule.substr(7);

                generated_group.Proxies.emplace_back(std::move(rule));
            }

            expanded_groups.emplace_back(std::move(generated_group));
        }
    }

    for (const auto &[parent_name, child_name]: parent_links) {
        auto parent = std::find_if(expanded_groups.begin(), expanded_groups.end(), [&](const ProxyGroupConfig &group) {
            return group.Name == parent_name;
        });
        if (parent == expanded_groups.end())
            continue;
...
std::string buildUrlGroupQuery(const std::string &group) {
    return group.empty() ? "" : "group=" + urlSafeBase64Encode(group) + "&";
}

std::string buildInlineGroupField(const std::string &group) {
    return group.empty() ? "" : ", group=" + group;
}
...
bool applyMatcher(const std::string &rule, std::string &real_rule, const Proxy &node) {
    ...
    static const std::string groupid_regex = R"(^!!(?:GROUPID|INSERT)=([\d\-+!,]+)(?:!!(.*))?$)", group_regex =
            R"(^!!(?:GROUP)=(.+?)(?:!!(.*))?$)", tag_regex = R"(^!!(?:TAG)=(.+?)(?:!!(.*))?$)", untagged_regex = R"(^!!UNTAGGED(?:!!(.*))?$)";
    ...
    if (startsWith(rule, "!!GROUP=")) {
        ...
        regGetMatch(rule, tag_regex, 3, 0, &target, &ret_real_rule);
        real_rule = ret_real_rule;
        return regFind(node.Tag, target);
    } else if (startsWith(rule, "!!UNTAGGED")) {
        regGetMatch(rule, untagged_regex, 2, 0, &ret_real_rule);
        real_rule = ret_real_rule;
                ...
                if (ss) {
                    ...
                    if (std::find(ssr_ciphers.begin(), ssr_ciphers.end(), method) != ssr_ciphers.end() &&
                        ...
                        proxyStr = "ssr://" + urlSafeBase64Encode(
                                       ...
                                       + "/?" + buildUrlGroupQuery(x.Group) + "remarks=" + urlSafeBase64Encode(
                                           remark));
                } else
                    continue;
                break;
        ...
        switch (x.Type) {
            ...
            case ProxyType::ShadowsocksR:
                if (ext.nodelist) {
                    ...
                    proxyStr = x.Remark + " = shadowsocksr, " + hostname + ", " + port + ", " + method + ", \"" +
                               password + "\"" + buildInlineGroupField(x.Group) + ", protocol=" + protocol + ", obfs=" + obfs;
                    if (!protoparam.empty())
            ...
            case ProxyType::Shadowsocks:
                if (ext.nodelist) {
                    ...
                    proxyStr += "#" + urlEncode(x.Remark);
                } else {
                    proxyStr =
                            x.Remark + " = shadowsocks, " + hostname + ", " + port + ", " + method + ", \"" +
                            password + "\"" + buildInlineGroupField(x.Group);
            ...
            case ProxyType::SOCKS5:
                proxyStr = x.Remark + " = socks, upstream-proxy-address=" + hostname + ", upstream-proxy-port=" +
                           port + buildInlineGroupField(x.Group);
                if (!username.empty() && !password.empty())
                    proxyStr += ", upstream-proxy-auth=true, upstream-proxy-username=" + username +
                            ", upstream-proxy-password=" + password;
                else
